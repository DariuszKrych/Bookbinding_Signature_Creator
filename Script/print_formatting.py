...
# Anchor to the project root (up two levels from Script/print_formatting.py)
ROOT_DIR = Path(__file__).resolve().parent.parent

font_path = ROOT_DIR / 'Script' / 'Baskervville-Regular.ttf'
pdfmetrics.registerFont(TTFont('Baskervville', str(font_path)))
...
def stamp_book(book_file_1, numbered_pdf_output, page_margins_inches, column_gap_inches, column_width_inches):
    ...
    while page_num < page_count + 1:
        # Use ROOT_DIR to anchor the path
        file_path = str(ROOT_DIR / "Script" / "page_num_stamps" / f"{page_num}.pdf")
    ...
    with open(book_file_1, 'rb') as book_file:
        ...
        for i, page in enumerate(writer.pages):
            page_number = i + 1
            # Use ROOT_DIR to anchor the path
            stamp_file_path = str(ROOT_DIR / "Script" / "page_num_stamps" / f"{page_number}.pdf")
    ...
    # Use ROOT_DIR to anchor the path
    page_num_stamps_dir = ROOT_DIR / "Script" / "page_num_stamps"
...
def split_to_signatures(book_file, signature_pages, signature_save_folder):
    ...
    for i, page in enumerate(reader.pages):
        ...
        print(
            f"Splitting up page {page_num} into pages {page_num + page_doubling_adjust} & {page_num + 1 + page_doubling_adjust}.")
    ...
    while True:
        ...
        right_side_num = (signature_pages * 4 * signature_num) - page_num + 1 + sig_jump_up_adjust - (
                    small_sig_save_adjust * 2)
        ...
        if pages_in_sig == (signature_pages_x2) + 1 - small_sig_save_adjust:
            # Keep folder joining safe using Path
            output_file_path = str(Path(signature_save_folder) / f"signature_{signature_num}.pdf")
